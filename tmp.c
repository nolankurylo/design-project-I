...
xQueueHandle xQueue_lightLength = 0;
...
int main(void)
{
	...
	xQueue_lightLength = xQueueCreate( 	mainQUEUE_LENGTH,		/* The number of items the queue can hold. */
								sizeof( uint16_t ) );	/* The size of each item the queue holds. */

	/* Add to the registry, for the benefit of kernel aware debugging. */
	vQueueAddToRegistry( xQueue_nextCar, "nextCarQueue" );
	vQueueAddToRegistry( xQueue_lightLength, "lightLengthQueue" );
...
static void Manager_Task( void *pvParameters ) // reading traffic rate task
{
	...
	while(1)
	{
		...
		if( xQueueSend(xQueue_lightLength, &flow_level,1000) )
		{
			vTaskDelay(1000);
		}
		else
		...
		if( xQueueSend(xQueue_nextCar,&next_car,1000) )
		{
			vTaskDelay(1000);
		}
		else
...
static void Traffic_Light_State_Task( void *pvParameters ) // reading traffic rate task
{
	...
	while(1)
	{
		if( xQueueReceive(xQueue_lightLength, &flow_level, 500) ) // get current flow level from queue push by manager
			{ // car received set length of green and red light
				vTaskDelay(250);
				//
...
static void Traffic_Task( void *pvParameters ) // waits for new cars to be sent in the queue and updates traffic display
{
	...
	while(1)
	{
		if( xQueueReceive(xQueue_nextCar, &next_car, 500) ) // get next car from queue push by manager
		{
			vTaskDelay(250);
			printf("adding new car: %d\n", next_car);
			moveTrafficRight(cars_array, next_car); // move traffic (in array)
		}
